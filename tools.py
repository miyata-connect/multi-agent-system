...
def call_auditor(plan_text: str) -> str:
    ...
    try:
        ...
        messages = [
            SystemMessage(content="あなたは冷徹な監査役です。ユーザーの計画に対し、技術的リスク、コスト超過リスク、実現可能性の懸念点のみを厳しく指摘してください。褒める必要はありません。日本語で回答してください。"),
...
def call_data_processor(text_data: str) -> str:
    ...
    try:
        ...
        messages = [
            SystemMessage(content="あなたは優秀なデータ処理係です。渡されたテキストデータを分析し、重要なポイントを要約して整理してください。日本語で回答してください。"),
